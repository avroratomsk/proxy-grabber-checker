        ...
        try:
            ...
            if str(response_json['country_code']) == 'RU':
                if str(response_json['subdivision1_name']) != 'None' or str(response_json['city_name']) != 'None':
                    ...
                    f = open('./result_http.txt', 'a', encoding="utf-8")
                    f.write(str(proxy['https']) + '|' + location + '\r')

            if str(response_json['country_code']) != 'RU':
                print(color.BOLD + color.YELLOW + "✔ " + color.END + color.BOLD + proxy['https'] + color.END + " (" + color.CYAN + str(response_json['subdivision1_name']).replace('́с','с') + color.END + ', ' + str(response_json['city_name']).replace('́с', 'с') + ")")
...
def get_proxy_list():
    proxies = []
    with open('./http.txt', "r", encoding="utf-8") as file:
        for line in file:
            try:
...
if __name__ == '__main__':
    ...
    f = open('./result_http.txt', 'w+', encoding="utf-8")
    f.write('')
    ...
    proxies = []
    with open('./result_http.txt', "r", encoding="utf-8") as file:
        for line in file:
            try:
                proxies.append(line.replace('\n', ''))
            except Exception as e:
